...
-- Running the parser/evaluator
-- ----------------------------

-- Take a series of strings and convert them to expressions
genExps :: [String] -> [Exp]
genExps = let pExp = (\(Right e) -> e) . parse anExp ""     -- Will FAIL if anExp not fully defined
          in  map pExp
...
prettyParserFails :: TestCase -> [String]
prettyParserFails (input, exps, _)
    =   [ "#### Test case"
        , ""
        , "```"
        ] ++ input ++
...
prettyEvaluatorFails :: TestCase -> [String]
prettyEvaluatorFails (input, exps, val)
    =   [ "#### Test case"
        , "```"
        ] ++ input ++
        [ "```"
        ...
        , ""
        , "#### Evaluates to:"
        , show $ genVals exps runtime
        , ""
        ]

-- Produce test-cases which don't pass
failsParser :: [TestCase] -> [TestCase]
failsParser = filter (not . passesParser)

failsEvaluator :: [TestCase] -> [TestCase]
failsEvaluator = filter (not . passesEvaluator)
...
runTests tests = let parsdOuts = concatMap (testSuite failsParser prettyParserFails) tests
                     evaldOuts = concatMap (testSuite failsEvaluator prettyEvaluatorFails) tests
                 in  mapM putStrLn $    [ ""
                                        , "Parser Tests"
                                        , "============"
                                        , ""
...
-- main :: IO [()]
-- main = mapM (putStrLn . genTestSuiteOut) testSuites
--
-- -- type TestCase = ([String], [Exp], String)
-- -- type TestSuite = (String, [TestCase], [String])
...
primitiveBools = ("Primitive Boolean Operators", [(["and","or","not"],[SymExp "and",SymExp "or",SymExp "not"],"[*primitive*,*primitive*,*primitive*]"),(["(and 't 't)","(and 't 'nil)","(and 't 't 't 't)","(and 't 'nil 6 4)","(and 't 5)","(and (> 4 2) (> 5 2))","(and (> 4 2) (> 2 5))"],[SExp [SymExp "and",SExp [SymExp "quote",SymExp "t"],SExp [SymExp "quote",SymExp "t"]],SExp [SymExp "and",SExp [SymExp "quote",SymExp "t"],SExp [SymExp "quote",SymExp "nil"]],SExp [SymExp "and",SExp [SymExp "quote",SymExp "t"],SExp [SymExp "quote",SymExp "t"],SExp [SymExp "quote",SymExp "t"],SExp [SymExp "quote",SymExp "t"]],SExp [SymExp "and",SExp [SymExp "quote",SymExp "t"],SExp [SymExp "quote",SymExp "nil"],IntExp 6,IntExp 4],SExp [SymExp "and",SExp [SymExp "quote",SymExp "t"],IntExp 5],SExp [SymExp "and",SExp [SymExp ">",IntExp 4,IntExp 2],SExp [SymExp ">",IntExp 5,IntExp 2]],SExp [SymExp "and",SExp [SymExp ">",IntExp 4,IntExp 2],SExp [SymExp ">",IntExp 2,IntExp 5]]],"[t,nil,t,nil,t,t,nil]"),(["(or 't 't)","(or 't 'nil)","(or 'nil 'nil)","(or 5 2 6)","(or (and 't 't) 'nil)","(or (and 'nil 't) 6)"],[SExp [SymExp "or",SExp [SymExp "quote",SymExp "t"],SExp [SymExp "quote",SymExp "t"]],SExp [SymExp "or",SExp [SymExp "quote",SymExp "t"],SExp [SymExp "quote",SymExp "nil"]],SExp [SymExp "or",SExp [SymExp "quote",SymExp "nil"],SExp [SymExp "quote",SymExp "nil"]],SExp [SymExp "or",IntExp 5,IntExp 2,IntExp 6],SExp [SymExp "or",SExp [SymExp "and",SExp [SymExp "quote",SymExp "t"],SExp [SymExp "quote",SymExp "t"]],SExp [SymExp "quote",SymExp "nil"]],SExp [SymExp "or",SExp [SymExp "and",SExp [SymExp "quote",SymExp "nil"],SExp [SymExp "quote",SymExp "t"]],IntExp 6]],"[t,t,t,t,t,t]"),(["(not (> 5 3))","(not (< 5 3))","(not 't 't)","(not 'nil 't 't 't)"],[SExp [SymExp "not",SExp [SymExp ">",IntExp 5,IntExp 3]],SExp [SymExp "not",SExp [SymExp "<",IntExp 5,IntExp 3]],SExp [SymExp "not",SExp [SymExp "quote",SymExp "t"],SExp [SymExp "quote",SymExp "t"]],SExp [SymExp "not",SExp [SymExp "quote",SymExp "nil"],SExp [SymExp "quote",SymExp "t"],SExp [SymExp "quote",SymExp "t"],SExp [SymExp "quote",SymExp "t"]]],"[nil,t,*** Scheme-Exception: Incorrect number of arguments. not form is unary.,*** Scheme-Exception: Incorrect number of arguments. not form is unary.]")],["Do you handle the boolean operators `and` and `or` correctly?","Do you handle the unary operator `not` correctl?"])
primitiveMisc :: TestSuite
primitiveMisc = ("Primitive Misc.", [(["()"],[SExp []],"[nil]"),(["eq?"],[SymExp "eq?"],"[*primitive*]"),(["(+)","(-)","(*)","(>)","(<)","(>=)","(<=)","(=)","(!=)","(eq?)","(and)","(or)","(not)","(list)","(car)","(cdr)"],[SExp [SymExp "+"],SExp [SymExp "-"],SExp [SymExp "*"],SExp [SymExp ">"],SExp [SymExp "<"],SExp [SymExp ">="],SExp [SymExp "<="],SExp [SymExp "="],SExp [SymExp "!="],SExp [SymExp "eq?"],SExp [SymExp "and"],SExp [SymExp "or"],SExp [SymExp "not"],SExp [SymExp "list"],SExp [SymExp "car"],SExp [SymExp "cdr"]],"[0,0,1,t,t,t,t,t,t,t,t,t,*** Scheme-Exception: Incorrect number of arguments. not form is unary.,nil,*** Scheme-Exception: Not a cons cell: [],*** Scheme-Exception: Not a cons cell: []]"),(["(eq? 5 5)","(eq? 6 5 6)","(eq? (+ 3 3) 6)","(eq? 'a 5)","(eq? 'a 'b)","(eq? 'a 'a 'b)","(eq? 'a 'a 'a)"],[SExp [SymExp "eq?",IntExp 5,IntExp 5],SExp [SymExp "eq?",IntExp 6,IntExp 5,IntExp 6],SExp [SymExp "eq?",SExp [SymExp "+",IntExp 3,IntExp 3],IntExp 6],SExp [SymExp "eq?",SExp [SymExp "quote",SymExp "a"],IntExp 5],SExp [SymExp "eq?",SExp [SymExp "quote",SymExp "a"],SExp [SymExp "quote",SymExp "b"]],SExp [SymExp "eq?",SExp [SymExp "quote",SymExp "a"],SExp [SymExp "quote",SymExp "a"],SExp [SymExp "quote",SymExp "b"]],SExp [SymExp "eq?",SExp [SymExp "quote",SymExp "a"],SExp [SymExp "quote",SymExp "a"],SExp [SymExp "quote",SymExp "a"]]],"[t,nil,t,nil,nil,nil,t]")],["Do you handle the empty form `()` correctly?","Do you handle the base-cases of the primitive operators correctly?","Do you handle `eq?` correctly?"])
consListCarCdr :: TestSuite
consListCarCdr = ("Cons, List, Car, Cdr", [(["cons","list","car","cdr"],[SymExp "cons",SymExp "list",SymExp "car",SymExp "cdr"],"[*** Scheme-Exception: Symbol cons has no value.,*primitive*,*primitive*,*primitive*]"),(["(list (> 3 4) 't 15 'nil (< 5 2 3 5))","(car (list 'a 'b))","(cdr (list 'a 'b))","(car (list 'a 'b 'c))","(cdr (list 'a 'b 'c))","(cdr (list 'a))","(cdr 'a)"],[SExp [SymExp "list",SExp [SymExp ">",IntExp 3,IntExp 4],SExp [SymExp "quote",SymExp "t"],IntExp 15,SExp [SymExp "quote",SymExp "nil"],SExp [SymExp "<",IntExp 5,IntExp 2,IntExp 3,IntExp 5]],SExp [SymExp "car",SExp [SymExp "list",SExp [SymExp "quote",SymExp "a"],SExp [SymExp "quote",SymExp "b"]]],SExp [SymExp "cdr",SExp [SymExp "list",SExp [SymExp "quote",SymExp "a"],SExp [SymExp "quote",SymExp "b"]]],SExp [SymExp "car",SExp [SymExp "list",SExp [SymExp "quote",SymExp "a"],SExp [SymExp "quote",SymExp "b"],SExp [SymExp "quote",SymExp "c"]]],SExp [SymExp "cdr",SExp [SymExp "list",SExp [SymExp "quote",SymExp "a"],SExp [SymExp "quote",SymExp "b"],SExp [SymExp "quote",SymExp "c"]]],SExp [SymExp "cdr",SExp [SymExp "list",SExp [SymExp "quote",SymExp "a"]]],SExp [SymExp "cdr",SExp [SymExp "quote",SymExp "a"]]],"[(nil t 15 nil nil ),a,(b ),a,(b c ),nil,*** Scheme-Exception: Not a cons cell: [a]]"),(["(cons 2 3)"],[SExp [SymExp "cons",IntExp 2,IntExp 3]],"[(2 . 3)]"),(["(cons 2 (cons 3 4))","(cons 2 (cons 3 (cons 4 'nil)))"],[SExp [SymExp "cons",IntExp 2,SExp [SymExp "cons",IntExp 3,IntExp 4]],SExp [SymExp "cons",IntExp 2,SExp [SymExp "cons",IntExp 3,SExp [SymExp "cons",IntExp 4,SExp [SymExp "quote",SymExp "nil"]]]]],"[(2 3 . 4),(2 3 4 )]"),(["(car (cons 2 (cons 3 (cons 4 'nil))))","(cdr (cons 2 (cons 3 (cons 4 'nil))))","(car (list 2 3 4 'a 'b))","(cdr (list 'b 'c 3 4 (+ 3 4)))"],[SExp [SymExp "car",SExp [SymExp "cons",IntExp 2,SExp [SymExp "cons",IntExp 3,SExp [SymExp "cons",IntExp 4,SExp [SymExp "quote",SymExp "nil"]]]]],SExp [SymExp "cdr",SExp [SymExp "cons",IntExp 2,SExp [SymExp "cons",IntExp 3,SExp [SymExp "cons",IntExp 4,SExp [SymExp "quote",SymExp "nil"]]]]],SExp [SymExp "car",SExp [SymExp "list",IntExp 2,IntExp 3,IntExp 4,SExp [SymExp "quote",SymExp "a"],SExp [SymExp "quote",SymExp "b"]]],SExp [SymExp "cdr",SExp [SymExp "list",SExp [SymExp "quote",SymExp "b"],SExp [SymExp "quote",SymExp "c"],IntExp 3,IntExp 4,SExp [SymExp "+",IntExp 3,IntExp 4]]]],"[2,(3 4 ),2,(c 3 4 7 )]")],["Are you handling primitives `list`, `car`, and `cdr` correctly?","Are you handling the `cons` form correctly?"])
